...
import { geoacListUrl } from "~/utils/geoacList";

const appConfig = useAppConfig();
const route = useRoute();
const { globalProfiles } = useGlobalProfiles();

const urlPath = ref(geoacListUrl(useGetInitialPageUrl()));
...
watch(() => route.fullPath, () => {
    urlPath.value = geoacListUrl(getPageUrl());
});
</script>
...
<template>
    <NuxtLayout sidepanel>
        ...
        <template #debug>
            <pre class="p-2"><b>{{ currentProfile?.title }}</b><br>{{ JSON.stringify(globalProfiles?.[currentProfile?.uri || ''], null, 2) }}</pre>
        </template>
        <template #breadcrumb>
